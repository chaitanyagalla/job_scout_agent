    ...
    if profile:
        ...
        llm_request.append_instructions([
            ...
            (
                ...
                "text extraction first and only uses Gemini as fallback."
            ),
            (
        ...
        if has_uploaded_artifact:
            llm_request.append_instructions([
                (
                    ...
                    "mode yet. First call `extract_resume_profile_from_artifact`, then "
                    "re-check with `get_resume_status` before searching or scoring."
                ),
            ])
...
4. If no resume profile is available:
   - If the user uploaded a resume or explicitly asked for resume-based matching,
     first call `extract_resume_profile_from_artifact`, then call `get_resume_status`
     in a separate tool round before deciding whether prompt-only mode is needed.
...
8. If resume-aware mode is active:
   - call `score_job_match` with the job title and full job description
...
- Only use `score_job_match` when a resume profile exists.
